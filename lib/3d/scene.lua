local PostFX = assert(SMODS.load_file("lib/3d/postfx.lua"))()
...
function Scene3D:update(dt)
    ...
    -- if love.keyboard.isDown("space") then move.y = move.y + 1 end
    -- if love.keyboard.isDown("lshift") then move.y = move.y - 1 end
    ...
    if move.x ~= 0 or move.y ~= 0 or move.z ~= 0 then
        ...
        if insideX and insideZ then
            ...
            local tryXInside =
                tryX.x > minX and tryX.x < maxX and
                tryX.z > minZ and tryX.z < maxZ

            local tryZInside =
                tryZ.x > minX and tryZ.x < maxX and
                tryZ.z > minZ and tryZ.z < maxZ
...
function Scene3D:draw()
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.shadow,
        { pos = Vec3.new(tp.x, tp.y - 0.05, tp.z), rot = tr },
        self.camera,
        rw,
        rh
    )
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.wood,
        { pos = Vec3.new(tp.x, tp.y, tp.z), rot = tr },
        self.camera,
        rw,
        rh
    )
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.rail,
        { pos = Vec3.new(tp.x, tp.y + 0.01, tp.z), rot = tr },
        self.camera,
        rw,
        rh
    )
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.felt,
        { pos = Vec3.new(tp.x, tp.y + 0.02, tp.z), rot = tr },
        self.camera,
        rw,
        rh
    )
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.leftWall,
        { pos = Vec3.new(tp.x - halfOuter, tp.y - wallH * 0.5, tp.z), rot = tr },
        self.camera,
        rw,
        rh
    )
    ...
    Renderer.collectMeshTriangles(
        tris,
        self.pokerTable.frontWall,
        { pos = Vec3.new(tp.x, tp.y - wallH * 0.5, tp.z + halfOuter), rot = tr },
        self.camera,
        rw,
        rh
    )
